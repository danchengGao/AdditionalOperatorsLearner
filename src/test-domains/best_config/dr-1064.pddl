...
(:action macro-BOARD-TRUCK-and-DRIVE-TRUCK-13
    :parameters (?driver_0 - driver ?truck_0 - truck ?loc_0 - location ?loc-from - location ?driver - driver)
    :precondition 
    (and
        (at ?truck_0 ?loc_0)
        (at ?driver_0 ?loc_0)
        ...
        (not (= ?loc_0 ?loc-from))
        (at ?truck_0 ?loc-from)
        (driving ?driver ?truck_0)
        (link ?loc-from ?loc-from)
    )
    :effect 
    (and
        (not (at ?driver_0 ?loc_0))
        (driving ?driver_0 ?truck_0)
        (not (empty ?truck_0))
        (not (at ?truck_0 ?loc-from))
        (at ?truck_0 ?loc-from)
    )
) 

(:action macro-DRIVE-TRUCK-and-DRIVE-TRUCK-174
    ...
    :effect 
    (and
        (not (at ?truck_0 ?loc-from_0))
        (not (at ?truck_0 ?loc-to_0))
        (at ?truck_0 ?loc-to)
    )
) 
