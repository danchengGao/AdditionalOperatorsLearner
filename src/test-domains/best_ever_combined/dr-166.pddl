...
(:action nonr-WALK-and-DISEMBARK-TRUCK-10
    :parameters (?driver_0 - driver ?loc-from_0 - location ?loc-to_0 - location ?truck - truck)
    :precondition 
    (and
        (at ?driver_0 ?loc-from_0)
        (path ?loc-from_0 ?loc-to_0)
        (not (= ?loc-from_0 ?loc-to_0))
        (at ?truck ?loc-from_0)
        (driving ?driver_0 ?truck)
    )
    :effect 
    (and
        (at ?driver_0 ?loc-to_0)
        (not (driving ?driver_0 ?truck))
        (at ?driver_0 ?loc-from_0)
        (empty ?truck)
    )
) 
