...
(:action nonr-move-curb-to-curb-and-move-car-to-car-82
    :parameters (?car_0 - car ?curbsrc_0 - curb ?carsrc - car)
    :precondition 
    (and
        (car-clear ?car_0)
        (curb-clear ?curbsrc_0)
        (at-curb-num ?car_0 ?curbsrc_0)
        (not (= ?car_0 ?carsrc))
        (behind-car ?car_0 ?carsrc)
        (at-curb ?car_0)
    )
    :effect 
    (and
        (not (curb-clear ?curbsrc_0))
        (curb-clear ?curbsrc_0)
        (at-curb-num ?car_0 ?curbsrc_0)
        (not (at-curb-num ?car_0 ?curbsrc_0))
        (not (car-clear ?car_0))
        (car-clear ?carsrc)
        (behind-car ?car_0 ?car_0)
        (not (behind-car ?car_0 ?carsrc))
    )
) 
